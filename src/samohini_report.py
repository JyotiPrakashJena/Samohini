...
def execute_selected_stocks():
    ...
    try:
        ...
        for selected_stock in selected_stocks:
            ...
            try:
                stock_current_mkt_price = main.get_stock_details(
                    stock_id
                )  # For Period=1day
                buy_price = format_float(value_from_dict(selected_stock, "buy_price"))
                current_price = format_float(
                    value_from_list(stock_current_mkt_price, "close")
                )
...
def generate_profit_loss_report():
    ...
    try:
        ...
        for executed_stock in executed_stocks:
            ...
            curr_price = format_float(
                value_from_dict(executed_stock, "current_market_price")
            )
