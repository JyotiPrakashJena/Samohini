from models.volume_model import (current_market_price,
...
class VolumeIndicator:
    ...
    def volume_indicator_response(self, request: request_volume_module) -> response_volume_module:
        ...
        volume_indicator = volume_indicators(volume_now=self.stock_data['volume'][-1],
                                             avg_volume=self.get_avg_volume(),
                                             vol_indicator_check=self.get_check_vol_indicator())
        volume_response = response_volume_module(stock_id=request.stock_id,
