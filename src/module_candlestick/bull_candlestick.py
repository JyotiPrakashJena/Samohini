...
class BullCandleStick:
    ...
    def bull_green_marubozu(self) -> bool:
        ...
        current = self.stock_data.iloc[-1]
        realbody = abs(current['open'] - current['close'])
        return stock_ratio(current['open'], current['low']) < 0.0015 and \
        stock_ratio(current['high'],current['close']) < 0.0015 and \
        realbody >= average_body(self.stock_data['open'], self.stock_data['close'])
